r"""
mutation_captions.py -- pulls Welsh YouTube captions (manual or auto-generated)
...
Usage:
    python mutation_captions.py <youtube_url_or_id>
...
def _extract_info_with_backoff(url, opts, max_retries=3, download=False):
    ...
    return youtube_call("caption download" if download else "caption discovery",
                        _extract, max_attempts=max_retries)
...
def run_corroboration(mutations_csv_path, captions_csv_path, nlp=None, cap_kind=None, output_path=None):
    ...
    prior mutation_captions.py run) and the video's own words_*.csv, and
