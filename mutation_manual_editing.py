r"""manual_editing.py -- interactive manual review tool for mutations_*.csv files.
...
Usage:
    python mutation_manual_editing.py
    ...
    python mutation_manual_editing.py path\to\mutations_20250101_120000_SomeVideo.csv
        -- review just this one file instead.

    python mutation_manual_editing.py --pick
...
def find_mutation_csvs(explicit_paths):
    ...
    # became separate files (see mutation_captions.run_corroboration's
...
def find_segments_csv(mutations_csv_path):
    ...
    exist (older files, or a layout that predates this fix).
    ...
    # run-then-video nesting -- mutations_csv_path now has two parent
    # levels to read off (slug, then stamp above it), not one flat
    # <stamp>_<slug> folder. Also switched the rglob fallback's search
    # root from a fixed mutations_csv_path.parents[N] hop count to
